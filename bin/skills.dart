/// Installs, updates, or checks the flutter_recorder agent skills.
///
/// Running `dart run flutter_recorder:skills` is itself the consent to write
...
// The bundled agent skills teaching correct flutter_recorder usage each live at
...
Future<void> main(List<String> args) async {
  if (args.contains('-h') || args.contains('--help')) {
    stdout.writeln(
      'Usage: dart run flutter_recorder:skills [--check]\n'
      '\n'
      'Installs or updates the flutter_recorder agent skills (correct-usage '
...
String describeSkillPlan(SkillInstallPlan plan) {
  switch (plan.action) {
    case SkillInstallAction.sourceMissing:
      return 'Could not locate the bundled flutter_recorder skills. This '
          'package version may predate them, or it is a git dependency '
          'published without them.';
    case SkillInstallAction.upToDate:
      return 'The flutter_recorder agent skills are up to date '
          '(${plan.skillNames.length} installed).';
    case SkillInstallAction.install:
      return '${plan.installCount} of the flutter_recorder agent skills '
          '(${plan.skillNames.length} total) are not installed. Install them '
          'with: dart run flutter_recorder:skills';
    case SkillInstallAction.update:
      return '${plan.updateCount} flutter_recorder agent skill(s) have a newer '
          'version available. Update with: dart run flutter_recorder:skills';
  }
}
...
Future<String> installSkills({Directory? projectRoot, Uri? skillsRoot}) async {
  ...
  final bundled = rootUri == null
      ...
      : _bundledSkills(rootUri);
  if (bundled.isEmpty) {
    return 'Could not locate the bundled flutter_recorder skills; skipped '
        'skill install.';
  }
...
Future<Uri?> _resolveBundledSkillsRoot() async {
  final lib = await Isolate.resolvePackageUri(
    Uri.parse('package:flutter_recorder/flutter_recorder.dart'),
  );
