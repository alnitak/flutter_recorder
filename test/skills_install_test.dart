...
void main() {
  ...
  setUp(() {
    tempDir = Directory.systemTemp.createTempSync('recorder_skills_test_');
  ...
  group('Agent Skills installer', () {
    ...
    test(
      ...
      () async {
        ...
        expect(
          result,
          contains(
            'Installed 8 flutter_recorder agent skills into .agents/skills.',
          ),
        );

        for (final skillName in expectedSkills) {
    ...
    test('detects stale versions and updates them', () async {
      ...
      expect(
        ...
        contains(
          '1 flutter_recorder agent skill(s) have a newer version available',
        ),
      );
    ...
    test('CLI --check exit code reflects installation state', () async {
      ...
      expect(
        ...
        contains('Installed 8 flutter_recorder agent skills'),
      );
      ...
      expect(
        ...
        contains(
          'The flutter_recorder agent skills are up to date (8 installed).',
        ),
      );
    });
  });
