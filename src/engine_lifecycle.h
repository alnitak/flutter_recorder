...
extern "C"
{
  ...
  FFI_PLUGIN_EXPORT bool flutter_recorder_prepareEngineInitForRequest(int64_t owner_engine_id,
                                                                     uint64_t shutdown_epoch);

  /// Retire every Dart callable owned by [engine_id].
  FFI_PLUGIN_EXPORT bool flutter_recorder_clearDartCallbackRegistrationsForEngine(
      int64_t engine_id);
